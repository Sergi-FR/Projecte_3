...
SELECT *
		from cursos c
		where c.ID = (select I.IDI
							from individuals I
                            where c.ID = I.IDI);
                            
SELECT *
		from cursos C
		where C.ID = (select CO.IDC
							from colectius CO
                            where C.ID = CO.IDC);
                            
select C.ID, C.nomMonitor, C.data_curs, COM.nivell_curs, COM.hora_inici, COM.hora_final, COM.preu
from cursos C, competicio COM
where C.ID= COM.ID_comp;
