...
public class CursCompeticio extends Curs {
    ...
    public ArrayList<CursCompeticio> consultaCursCompeticioBD() throws SQLException {
        ...
        // Demanem el DNI
        String consultaColectius = "SELECT C.ID, C.nomMonitor, C.data_curs AS data_inici, CO.data_final, CO.nivell_curs ,CO.hora_inici, CO.hora_final, CO.preu FROM competicio CO, cursos C WHERE C.ID = CO.ID_comp";
        ...
        while (rs.next()) {
            ...
            colectiu.setData(Date.valueOf(rs.getString("data_inici")).toLocalDate());
            colectiu.setData_fi(Date.valueOf(rs.getString("data_final")).toLocalDate());
